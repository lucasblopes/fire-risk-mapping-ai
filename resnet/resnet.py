...
# ==============================================================================
# Options: "RandomForest", "XGBoost", "LightGBM", "ResNet"
MODEL_NAME = "ResNet"
MONTHS_TO_GENERATE = [1, 3, 5, 6, 7, 8, 9, 11]  # Jan, Mar, May, Jun, Jul, Aug, Sep, Nov
# ==============================================================================

# Define file paths and model-specific output directories
TRAIN_FILE_PATH = "iberfire_train.csv"
TEST_FILE_PATH = "iberfire_test.csv"
FULL_2024_PATH = "iberfire_2024.csv"
FIRERISK_MAPS_DIR = Path(f"./fire_risk_maps_{MODEL_NAME}")
FIGURES_DIR = Path(f"./images/{MODEL_NAME}")
METRICS_DIR = Path("./metrics")

# Create output directories
FIRERISK_MAPS_DIR.mkdir(exist_ok=True)
FIGURES_DIR.mkdir(parents=True, exist_ok=True)
METRICS_DIR.mkdir(exist_ok=True)

print(f"IberFire Risk Mapping Pipeline (using PyTorch {MODEL_NAME})")
print("=" * 50)
...
# --- ResNet-based CNN with square reshape ---
import math
...
class PyTorchResNet(nn.Module):
    def __init__(self, n_features):
        ...
        # Compute square reshape and padding
        self.side = int(math.ceil(math.sqrt(n_features)))
        self.pad = self.side**2 - n_features

        # Base ResNet18 (can change to resnet34 if needed)
    ...
    def forward(self, x):
        ...
        if self.pad > 0:
            x = F.pad(x, (0, self.pad))  # pad zeros to make it square
...
class PyTorchCNNWrapper:
    ...
    def fit(self, X, y):
        ...
        for epoch in range(self.epochs):
            ...
            if (epoch + 1) % 5 == 0:
                print(
                    f"    Epoch [{epoch + 1}/{self.epochs}] - Loss: {epoch_loss / len(loader):.4f}"
                )
        return self
...
models = {
    "RandomForest": RandomForestClassifier(
        n_estimators=100, max_depth=10, random_state=42, n_jobs=-1, min_samples_leaf=20
    ),
    ...
    "LightGBM": lgb.LGBMClassifier(
        objective="binary", n_estimators=100, max_depth=5, random_state=42, n_jobs=-1
    ),
    "ResNet": PyTorchCNNWrapper(PyTorchResNet, n_features=n_features, epochs=100),
}
...
# ==============================================================================
# 5. Train Final Model, Evaluate, and Save Metrics
# ==============================================================================
...
# ==============================================================================
# 6. Generate Monthly Fire Risk GeoTIFF Maps
# ==============================================================================
...
print("\n" + "=" * 50)
print(f"    Pipeline completed using model: {MODEL_NAME}")
print("=" * 50)
